...
function Stat({ icon: Icon, label, value, detail }: { icon: typeof CloudRain; label: string; value: string; detail: string }) {
  return (
    <div className="rounded-2xl bg-[#f7faf9] p-3">
      <Icon className="h-5 w-5 text-[#277b6b]" />
      <p className="mt-3 text-[11px] font-bold text-[#6f8880]">{label}</p>
      <p className="mt-0.5 text-xl font-black tracking-[-0.04em]">{value}</p>
      <p className="mt-0.5 text-[10px] font-semibold text-[#719087]">{detail}</p>
    </div>
  );
}
...
export function VictimNavigation({ current }: { current: "home" | "track" | "safety" | "more" }) {
  ...
  const items = [
    ...
    { id: "safety" as const, icon: ShieldCheck, label: t("Safety"), path: "/safety" },
    { id: "more" as const, icon: MoreHorizontal, label: t("More"), path: "/more" },
  ];
  return (
    <nav
      aria-label="Victim App navigation"
      className="fixed inset-x-0 bottom-0 z-40 mx-auto flex max-w-lg justify-around border-t border-[#e7edeb] bg-white/95 px-4 pb-[max(0.7rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur md:bottom-6 md:rounded-[1.6rem] md:border md:shadow-xl"
    >
      {items.map(({ id, icon: Icon, label, path }) => (
        <button
          key={id}
          onClick={() => setLocation(path)}
          className={`grid min-w-16 place-items-center gap-1 rounded-xl px-3 py-1.5 text-[10px] font-extrabold ${
            current === id ? "text-[#df3e43]" : "text-[#7b8f88]"
          }`}
        >
          <span className={`grid h-8 w-8 place-items-center rounded-xl ${current === id ? "bg-[#fff0ef]" : ""}`}>
