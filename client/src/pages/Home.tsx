...
export default function Home() {
  ...
  return <div className="min-h-screen bg-[#f6f8f7] text-[#142c2b] dark:bg-[#081b18] dark:text-[#e6f5ef]"><main className="relative mx-auto min-h-screen max-w-lg overflow-hidden bg-[#fcfdfd] px-5 pb-28 pt-6 shadow-2xl shadow-[#113c35]/10 dark:bg-[#102622] dark:shadow-black/30 md:my-6 md:min-h-[850px] md:rounded-[2.75rem] md:border">
...
function VoiceNoteCard() {
  ...
  return <section className="mt-8 rounded-[1.55rem] bg-white p-3 shadow-[0_12px_28px_rgba(22,60,53,.09)] ring-1 ring-black/[.035] dark:bg-[#17332d] dark:ring-white/10"><div className="flex items-center gap-3"><button onClick={recording ? stop : start} className={`grid h-12 w-12 shrink-0 place-items-center rounded-2xl text-white transition active:scale-95 ${recording ? "bg-[#df3e43]" : "bg-[#174e46]"}`}>{recording ? <Square className="h-4 w-4 fill-current" /> : <Mic className="h-5 w-5" />}</button><button onClick={recording ? stop : start} className="min-w-0 flex-1 text-left"><p className="text-sm font-extrabold">{recording ? t("Stop and save voice note") : draft ? t("Voice note ready") : t("Record voice note")}</p><p className="mt-0.5 text-xs text-[#708881] dark:text-[#b5cdc5]">{draft ? `${draft.durationSeconds}s · ${t("Attached to your next SOS")}` : t("Speak briefly if typing is difficult")}</p></button>{draft && !recording && <button onClick={() => { clearSosVoiceNote(); setDraft(null); setMessage(""); }} aria-label={t("Remove voice note")} className="rounded-xl bg-[#f3f6f5] px-3 py-2 text-[11px] font-bold text-[#617970] dark:bg-[#264a41] dark:text-[#d5e9e1]">{t("Clear")}</button>}</div>{message && <p className="px-2 pb-1 pt-3 text-[11px] font-semibold text-[#54746b] dark:text-[#b5cdc5]">{message}</p>}</section>;
}
...
function LocationPreview({ point, state }: { point: Point; state: "finding" | "ready" | "unavailable" }) {
  ...
  return <section className="mt-5 overflow-hidden rounded-[1.55rem] bg-white p-3 shadow-[0_12px_28px_rgba(22,60,53,.09)] ring-1 ring-black/[.035] dark:bg-[#17332d] dark:ring-white/10"><div className="relative overflow-hidden rounded-[1.15rem]"><MapView key={pointKey} className="h-44" initialCenter={{ lat: point.latitude, lng: point.longitude }} initialZoom={14} onMapReady={map => { new window.google!.maps.marker.AdvancedMarkerElement({ map, position: { lat: point.latitude, lng: point.longitude }, title: "Your location" }); }} /><div className="pointer-events-none absolute left-3 top-3 rounded-lg bg-white/95 px-2.5 py-1 text-xs font-extrabold shadow-sm dark:bg-[#17332d]/95">{t("Map preview")}</div></div><div className="flex items-center justify-between px-1 pt-3"><span className="flex items-center gap-2 text-xs font-bold text-[#2d514a] dark:text-[#d5e9e1]"><MapPin className="h-4 w-4 text-[#df3e43]" />{state === "ready" ? t("Your current location") : state === "finding" ? t("Finding your location") : t("Map centered on Assam")}</span><span className="font-mono text-[10px] font-bold text-[#6a867e] dark:text-[#b5cdc5]">{point.latitude.toFixed(4)}° N, {point.longitude.toFixed(4)}° E</span></div></section>;
}
