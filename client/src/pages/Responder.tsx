...
import { Bell, CheckCircle2, ClipboardList, ClipboardPenLine, MapPinned, Navigation, UserRoundCheck } from "lucide-react";
  ...
  return <DashboardLayout navItems={nav} workspace="Rescuer operations" roleLabel="Field responder"><div className="space-y-6">
    {location === "/responder/map" ? <section><PageHeading eyebrow="Field map" title="See active conditions in your response area." /><OperationsMap layers={layers.data} /></section>
      : location === "/responder/alerts" ? <AlertsView items={alerts.data?.items ?? []} onRead={id => markRead.mutate({ notificationId: id })} />
          : <><section className="grid gap-5 lg:grid-cols-[1.25fr_0.75fr]"><div className="rounded-3xl bg-[#174e46] p-6 text-white shadow-[0_20px_60px_-30px_rgb(21_78_70/0.75)]"><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#b1dbd1]">Field readiness</p><h1 className="mt-2 text-2xl font-extrabold tracking-tight">{profile.data?.callSign || "Rescuer profile pending"}</h1><p className="mt-2 max-w-lg text-sm leading-6 text-[#c2e1d9]">Updates stay live while this workspace is open. Set availability before deploying and share GPS only when safe.</p><div className="mt-5 flex flex-wrap gap-2">{([['available','Available'],['on_mission','On mission'],['off_duty','Off duty']] as const).map(([value, label]) => <button key={value} onClick={() => withCurrentLocation(value)} disabled={setAvailability.isPending || !profile.data} className={`rounded-xl px-4 py-2.5 text-sm font-extrabold transition ${profile.data?.availability === value ? 'bg-white text-[#174e46]' : 'bg-white/10 text-[#d8eee8] hover:bg-white/20'}`}>{setAvailability.isPending ? "Updating…" : label}</button>)}</div></div><AlertSetup unread={alerts.data?.unread ?? 0} state={pushState} detail={pushDetail} disabled={subscribePush.isPending || pushState === "subscribed"} onEnable={enableAlerts} /></section><section><PageHeading eyebrow="My mission board" title="Work each mission through its required sequence." /><div className="grid gap-3">{missions.data?.length ? missions.data.map(({ mission, incident }) => <article key={mission.id} className="grid gap-4 rounded-2xl border bg-white p-5 shadow-sm md:grid-cols-[1fr_auto]"><div><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-xs font-medium text-primary">{incident.publicCode}</span><StatusBadge status={mission.status} /></div><h2 className="mt-2 text-lg font-extrabold">{incident.locationLabel}</h2><p className="mt-1 text-sm text-muted-foreground">{incident.peopleAffected} people · {incident.emergencyType} · <strong className={incident.severity === "critical" ? "text-destructive" : ""}>{incident.severity}</strong> priority</p>{mission.notes && <p className="mt-3 rounded-lg bg-muted px-3 py-2 text-xs leading-5 text-muted-foreground">{mission.notes}</p>}</div><div className="flex items-center md:justify-end">{mission.status === "resolved" ? <span className="flex items-center gap-2 text-sm font-bold text-[#19755f]"><CheckCircle2 className="h-5 w-5" /> Completed</span> : <Button disabled={updateMission.isPending} onClick={() => updateMission.mutate({ missionId: mission.id, status: mission.status === "pending" ? "dispatched" : "resolved" })} className="rounded-xl bg-primary text-primary-foreground hover:bg-primary/90">{updateMission.isPending ? "Updating…" : mission.status === "pending" ? <><Navigation className="mr-2 h-4 w-4" /> Mark dispatched</> : <><CheckCircle2 className="mr-2 h-4 w-4" /> Mark resolved</>}</Button>}</div></article>) : <Empty text="No mission has been assigned to this account. Keep your availability current so dispatchers can find you." />}</div></section></>}
  </div></DashboardLayout>;
}

function AlertSetup({ unread, state, detail, disabled: _disabled, onEnable }: { unread: number; state: PushState; detail: string; disabled: boolean; onEnable: () => void }) { const label = state === "subscribed" ? "Browser alerts active" : state === "permission_granted" ? "Finish browser alert setup" : state === "unsupported" ? "Background alerts unavailable" : "Enable browser alerts"; const isLocked = state === "subscribed" || state === "unsupported"; return <div className="rounded-3xl border bg-white p-6"><div className="flex items-center justify-between"><span className="grid h-10 w-10 place-items-center rounded-xl bg-secondary text-primary"><Bell className="h-5 w-5" /></span><span className="rounded-full bg-[#fff0ee] px-2.5 py-1 font-mono text-[10px] font-bold text-[#b44742]">{unread} NEW</span></div><h2 className="mt-5 text-lg font-extrabold">Assignment alerts</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">Register this device to receive mission assignments and nearby priority SOS alerts even when sudo MakeItWork is not open.</p><button type="button" onClick={onEnable} disabled={isLocked} className="mt-4 w-full rounded-xl border border-[#91cbbb] px-4 py-2.5 text-sm font-bold text-primary transition hover:bg-[#effaf6] disabled:cursor-not-allowed disabled:opacity-50">{label}</button>{detail && <p className={`mt-3 text-xs leading-5 ${state === "failed" ? "text-destructive" : state === "subscribed" ? "text-[#19755f]" : "text-muted-foreground"}`}>{detail}</p>}{!isLocked && <button type="button" onClick={onEnable} className="mt-2 w-full text-xs font-bold text-primary underline underline-offset-4">Retry browser alert registration</button>}</div>; }

function RescuerRegistration() { const [, setLocation] = useLocation(); const [phone, setPhone] = useState(""); const [note, setNote] = useState(""); const request = trpc.rescue.rescuer.requestRegistration.useMutation(); return <div className="min-h-screen app-grid flex items-center justify-center p-5"><section className="w-full max-w-lg rounded-[2rem] border bg-white p-6 shadow-xl"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#e7f7ee] text-primary"><ClipboardPenLine className="h-6 w-6" /></span><p className="mt-5 font-mono text-[10px] uppercase tracking-[0.18em] text-primary">Join the field team</p><h1 className="mt-1 text-2xl font-extrabold tracking-tight">Request rescuer access</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">Send your details to the Command Centre. An administrator will review and assign a secure field call sign before missions can be shown.</p><div className="mt-6 grid gap-4"><div className="grid gap-2"><Label htmlFor="phone">Phone</Label><Input id="phone" value={phone} onChange={event => setPhone(event.target.value)} placeholder="Optional field contact" /></div><div className="grid gap-2"><Label htmlFor="note">Experience or availability note</Label><Textarea id="note" value={note} onChange={event => setNote(event.target.value)} placeholder="Example: trained boat operator, medical volunteer, local area knowledge…" /></div></div><Button disabled={request.isPending} onClick={() => request.mutate({ phone: phone.trim() || undefined, note: note.trim() || undefined })} className="mt-6 w-full rounded-xl">{request.isPending ? "Sending request…" : "Request rescuer access"}</Button>{request.isSuccess && <p className="mt-4 rounded-xl bg-[#e7f7ee] p-3 text-sm font-semibold text-[#19755f]">Request sent. Wait for Command Centre approval, then refresh this page.</p>}{request.error && <p className="mt-4 text-sm font-semibold text-destructive">{request.error.message}</p>}<Button variant="outline" onClick={() => setLocation("/")} className="mt-3 w-full rounded-xl">Return to safety hub</Button></section></div>; }
...
function describePushFailure(error: unknown) { const message = error instanceof Error ? error.message : ""; if (/could not retrieve the public key/i.test(message)) return "This browser’s PushManager key store did not respond. sudo MakeItWork’s VAPID pair is valid. Restart the browser or device, then retry; if it persists, clear this site’s storage and notification permission or use another current browser. In-app alerts remain available."; if (/no active Service Worker/i.test(message)) return "The alert worker is still starting. Wait a few seconds, refresh this page, and try again."; return message || "We could not complete browser alert setup. Please try again or use the in-app Alerts view."; }
