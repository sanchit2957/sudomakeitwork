...
export function RequestDetails({ publicCode, active }: { publicCode: string; active: boolean }) {
  ...
  if (!user) return <button onClick={() => startLogin()} className="mt-4 w-full rounded-2xl border border-[#b8ded4] bg-[#f8fcfa] px-4 py-3 text-sm font-extrabold text-primary">Sign in to add request details</button>;
  if (!open) return <button onClick={() => setOpen(true)} className="mt-4 flex w-full items-center justify-between rounded-2xl border border-[#b8ded4] bg-[#f8fcfa] p-4 text-left"><span><span className="block text-sm font-extrabold text-[#173d37]">Add more details</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">People with you, help needed, and useful updates for responders.</span></span><UsersRound className="h-5 w-5 text-primary" /></button>;
  if (details.error) return null;
  return <section className="mt-4 rounded-2xl border border-[#b8ded4] bg-[#f8fcfa] p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="text-base font-extrabold text-[#173d37]">Add request details</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">Your SOS is already active. These updates help the assigned team prepare.</p></div><button onClick={() => setOpen(false)} className="text-xs font-extrabold text-primary">Close</button></div><div className="mt-4"><p className="text-xs font-bold text-[#58746c]">People needing help</p><div className="mt-2 flex items-center gap-3"><Button type="button" variant="outline" onClick={() => setPeopleAffected(value => Math.max(1, value - 1))} className="h-10 w-10 rounded-xl p-0">−</Button><output className="grid h-10 min-w-12 place-items-center rounded-xl bg-white text-lg font-black text-primary">{peopleAffected}</output><Button type="button" variant="outline" onClick={() => setPeopleAffected(value => Math.min(500, value + 1))} className="h-10 w-10 rounded-xl p-0">+</Button></div></div><div className="mt-4"><p className="text-xs font-bold text-[#58746c]">What help do you need?</p><div className="mt-2 grid grid-cols-2 gap-2">{([['flood','Flood rescue'],['medical','Medical help'],['trapped','Trapped / evacuation'],['evacuation','Shelter / relocation'],['other','Other']] as const).map(([value, label]) => <button key={value} onClick={() => setEmergencyType(value)} className={`rounded-xl border px-3 py-2.5 text-left text-xs font-extrabold ${emergencyType === value ? "border-primary bg-white text-primary" : "border-transparent bg-white text-[#5f776f]"}`}>{label}</button>)}</div></div><div className="mt-4 grid gap-3"><Textarea value={helpNeeds} onChange={event => setHelpNeeds(event.target.value)} maxLength={1000} placeholder="For example: boat needed, urgent medicine, elderly person, food and water" className="min-h-18 bg-white text-xs" /><Textarea value={notes} onChange={event => setNotes(event.target.value)} maxLength={2000} placeholder="Optional extra detail for the rescue team" className="min-h-18 bg-white text-xs" /></div><Button disabled={update.isPending || details.isLoading} onClick={() => update.mutate({ publicCode, peopleAffected, emergencyType, helpNeeds: helpNeeds.trim() || undefined, notes: notes.trim() || undefined })} className="mt-4 w-full rounded-xl bg-[#174e46]">{update.isPending ? "Saving update…" : "Save details for rescue team"}</Button>{update.error && <p className="mt-2 text-xs font-semibold text-destructive">{update.error.message}</p>}</section>;
}
