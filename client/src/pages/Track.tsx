...
export default function Track() {
  ...
  const status = trpc.rescue.emergency.statusByCode.useQuery(
    ...
    { enabled: /^SOS-[A-Z0-9]{8}$/.test(submittedCode), refetchInterval: 5_000, retry: false }
  );
...
export function AssignedRescuerCard({ rescuer, hospital }: { rescuer: AssignedRescuer; hospital?: { id: number; name: string; address: string; contactPhone: string | null; specialty: string; latitude: number; longitude: number } | null }) {
  const { t } = useLanguage();
  return (
    <section className="mt-6 overflow-hidden rounded-2xl border border-[#b8ded4] bg-[#f8fcfa]">
      ...
      <div className="border-t border-[#cbe8df] bg-white/70 p-4">
        <div className="flex items-center gap-2">
          <Radio className={`h-4 w-4 ${rescuer.locationStatus === "live" ? "animate-pulse text-primary" : "text-muted-foreground"}`} />
          <p className="text-sm font-extrabold text-[#173d37]">
            {rescuer.locationStatus === "live" ? t("Live rescuer location · updating every 5 seconds") : rescuer.locationStatus === "paused" ? t("Waiting for the next automatic location update") : t("Location sharing starts automatically after assignment")}
          </p>
        </div>
        {rescuer.location ? (
          <>
            ...
            <LiveRescuerMap latitude={rescuer.location.latitude} longitude={rescuer.location.longitude} destination={rescuer.destination} hospital={hospital} />
          </>
        ) : (
...
function LiveRescuerMap({ latitude, longitude, destination, hospital }: { latitude: number; longitude: number; destination?: { latitude: number; longitude: number }; hospital?: { name: string; latitude: number; longitude: number } | null }) {
  const { t } = useLanguage();
  ...
  }, [rescuerPosition.lat, rescuerPosition.lng, targetPosition?.lat, targetPosition?.lng, t]);
  ...
  const handleLeafletReady = useCallback(async (lMap: any) => {
    ...
    if (targetPosition) {
      const destIcon = L.divIcon({
        className: "dest-pin",
        html: `<div style="background:${hospital ? "#0f766e" : "#c94b45"};color:#fff;border-radius:50%;width:30px;height:30px;display:grid;place-items:center;border:2px solid #fff;box-shadow:0 3px 8px rgba(0,0,0,0.3);font-size:14px;">${hospital ? "🏥" : "🚨"}</div>`,
        iconSize: [30, 30],
        iconAnchor: [15, 15],
      });
      ...
      const route = L.polyline([[latitude, longitude], [targetPosition.lat, targetPosition.lng]], {
        color: "#d23f43",
        weight: 6,
        opacity: 0.85,
        dashArray: "8, 8",
      ...
      lMap.fitBounds(L.latLngBounds([[latitude, longitude], [targetPosition.lat, targetPosition.lng]]), { padding: [40, 40] });
      setRouteSummary("Route active.");
    }
  }, [latitude, longitude, targetPosition, hospital, t]);
