...
function VerifiedMedicalCare({ hospitals, loading, locationCopy, refresh }: { hospitals: Array<HospitalResource & { distance: number | null }>; loading: boolean; locationCopy: string; refresh: () => void }) { const { t } = useLanguage(); return <section className="mt-5"><div className="flex items-end justify-between gap-3"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[.16em] text-[#277b6b]">{t("Verified medical care")}</p><h2 className="mt-1 text-xl font-black">{t("Nearby hospitals")}</h2><p className="mt-1 text-xs font-semibold text-[#607c73]">{locationCopy}</p></div><button type="button" aria-label={t("Refresh nearby hospitals")} onClick={refresh} className="grid h-11 w-11 place-items-center rounded-2xl border border-[#b7d8ce] bg-white text-[#277b6b] active:scale-95"><RefreshCw className="h-4 w-4" /></button></div>{loading ? <HospitalLoading /> : hospitals.length ? <div className="mt-4 grid gap-4"><HospitalCard hospital={hospitals[0]} primary />{hospitals.slice(1, 4).map(hospital => <HospitalCard hospital={hospital} key={hospital.id} />)}</div> : <NoHospitalCard />}</section>; }
function HospitalCard({ hospital, primary = false }: { hospital: HospitalResource & { distance: number | null }; primary?: boolean }) { const { t } = useLanguage(); return <article className={`overflow-hidden rounded-[1.6rem] border shadow-[0_12px_28px_rgba(22,60,53,.07)] ${primary ? "border-[#9fcfc0] bg-white" : "border-[#e2ebe7] bg-white"}`}><div className={`p-4 ${primary ? "bg-[#eaf7f2]" : "bg-[#f8fbfa]"}`}><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 gap-3"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#174e46] text-white"><Hospital className="h-6 w-6" /></span><div className="min-w-0"><div className="flex items-start gap-2"><p className="min-w-0 flex-1 text-base font-black leading-5 text-[#163d36]">{hospital.name}</p><a href={`https://www.google.com/maps/dir/?api=1&destination=${hospital.latitude},${hospital.longitude}`} target="_blank" rel="noopener noreferrer" aria-label={t("Get directions")} title={`Open Google Maps directions to ${hospital.name}`} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-[#9fcfc0] bg-white text-[#277b6b] transition active:scale-95"><Navigation className="h-4 w-4" /></a></div><p className="mt-1 text-xs leading-5 text-[#58736a]">{hospital.distance !== null ? `${hospital.distance.toFixed(1)} km away · ` : ""}{hospital.address}</p></div></div><span className="rounded-full bg-white px-2.5 py-1 text-[9px] font-black uppercase text-[#197654]">{t("Open")}</span></div></div><div className="p-4"><div className="grid grid-cols-2 gap-3"><MetricTile icon={BedDouble} label={t("Beds ready")} value={hospital.availableEmergencyBeds} total={hospital.totalEmergencyBeds} tone="green" /><MetricTile icon={HeartPulse} label={t("ICU ready")} value={hospital.availableIcuBeds} total={hospital.totalIcuBeds} tone="blue" /></div><div className="mt-3 grid grid-cols-2 gap-2">{([{ label: t("Food"), value: hospital.foodSupplyStatus, icon: Utensils }, { label: t("Medicine"), value: hospital.medicineSupplyStatus, icon: Pill }, { label: t("Water"), value: hospital.waterSupplyStatus, icon: GlassWater }, { label: t("Power"), value: hospital.powerBackupStatus, icon: PlugZap }] as const).map(({ label, value, icon: Icon }) => <span key={label} className={`flex items-center gap-2 rounded-xl px-2.5 py-2 text-[10px] font-black ${supplyTone(value)}`}><Icon className="h-3.5 w-3.5" />{label}: {t(humanStatus(value))}</span>)}</div><div className="mt-3 flex items-center justify-between gap-3 border-t border-[#e6eeea] pt-3"><span className="flex min-w-0 items-center gap-1.5 text-[10px] font-semibold text-[#688078]"><Clock3 className="h-3.5 w-3.5 shrink-0" />{t("Updated")} {new Date(hospital.updatedAt).toLocaleString()}</span>{hospital.contactPhone ? <a href={`tel:${hospital.contactPhone}`} className="inline-flex shrink-0 items-center rounded-xl bg-[#174e46] px-3 py-2 text-xs font-black text-white"><PhoneCall className="mr-1.5 h-3.5 w-3.5" />{t("Call")}</a> : <span className="inline-flex shrink-0 items-center rounded-xl bg-[#eef2f0] px-3 py-2 text-xs font-black text-[#6d7f78]">{t("No call number")}</span>}</div></div></article>; }
function MetricTile({ icon: Icon, label, value, total, tone }: { icon: typeof BedDouble; label: string; value: number; total: number; tone: "green" | "blue" }) { const { t } = useLanguage(); const styles = tone === "green" ? "bg-[#e7f6ef] text-[#197654]" : "bg-[#eaf2fb] text-[#255c7d]"; return <div className={`rounded-2xl p-3 ${styles}`}><Icon className="h-5 w-5" /><p className="mt-2 text-2xl font-black tracking-[-.05em]">{value}</p><p className="text-[10px] font-black uppercase tracking-wide">{label}</p><p className="mt-0.5 text-[10px] font-semibold opacity-75">{t("of")} {total} {t("total")}</p></div>; }
function HospitalLoading() { const { t } = useLanguage(); return <div className="mt-4 rounded-[1.6rem] border border-dashed bg-white p-8 text-center"><Hospital className="mx-auto h-8 w-8 animate-pulse text-[#277b6b]" /><p className="mt-3 text-sm font-bold text-[#58736a]">{t("Loading verified hospitals…")}</p></div>; }
