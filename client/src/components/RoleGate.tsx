...
export function RoleGate({ roles, children }: { roles: Array<"user" | "rescuer" | "admin">; children: React.ReactNode }) {
  ...
  if (!roles.includes(user.role)) {
    ...
    return <div className="min-h-screen app-grid p-6"><div className="absolute right-4 top-4"><LanguageSelector compact /></div><section className="m-auto max-w-md rounded-[2rem] border bg-card p-8 text-center shadow-xl"><ShieldAlert className="mx-auto h-8 w-8 text-destructive" /><h1 className="mt-4 text-xl font-extrabold">Use {requestedRole} account</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">This signed-in account is registered as <strong>{user.role}</strong>. {nextStep}</p><Button onClick={logout} className="mt-6 w-full">{t("dashboard.signOut")}</Button><Button onClick={() => setLocation("/")} variant="outline" className="mt-3 w-full">{t("general.safetyHub")}</Button></section></div>;
  }
  return <>{children}</>;
}
