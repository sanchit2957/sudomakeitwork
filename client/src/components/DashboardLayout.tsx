...
export default function DashboardLayout({ children, navItems, workspace, roleLabel }: DashboardLayoutProps) {
  ...
  if (!user) return (
    <div className="min-h-screen app-grid flex items-center justify-center p-6">
      <section className="max-w-md rounded-[2rem] border bg-card p-8 text-center shadow-[0_24px_70px_-35px_rgb(22_86_75/0.45)]">
        ...
        <p className="font-mono text-xs uppercase tracking-[0.22em] text-primary">Secure access</p>
        <h1 className="mt-3 text-2xl font-extrabold tracking-tight">Sign in to open {workspace}</h1>
        ...
        <Button onClick={() => startLogin()} className="mt-7 w-full bg-primary text-primary-foreground hover:bg-primary/90">Continue securely</Button>
      </section>
    </div>
  );
    ...
    <SidebarProvider>
      <Sidebar className="border-r border-r-border bg-[#f7fbf9]">
        ...
        <SidebarFooter className="p-4">
          <div className="rounded-2xl bg-secondary/70 p-2 group-data-[collapsible=icon]:bg-transparent">
            ...
            <button onClick={logout} className="mt-2 flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-xs font-semibold text-muted-foreground hover:bg-white hover:text-destructive group-data-[collapsible=icon]:hidden"><LogOut className="h-3.5 w-3.5" /> Sign out</button>
          </div>
        </SidebarFooter>
      </Sidebar>
      <SidebarInset className="min-h-screen bg-background">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border/70 bg-background/85 px-4 backdrop-blur md:px-7">
          ...
          <div className="flex items-center gap-2 rounded-full bg-[#d9f1e7] px-3 py-1.5 text-xs font-bold text-[#176154]"><span className="h-2 w-2 rounded-full bg-[#1c9b75]" /> Live</div>
        </header>
