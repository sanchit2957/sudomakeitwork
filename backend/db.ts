...
import { emergencyContacts, EmergencyContact, InsertEmergencyContact, InsertUser, users, roleAccessCodes, RoleAccessCode } from "../database/schema";
...
export async function ensureRescuerProfile(userId: number, callSign = "NDRF Boat 4") {
  ...
  if (db) {
    try {
      const { rescueProfiles } = await import("../database/schema");
...
export async function ensureHospitalStaffProfile(userId: number) {
  ...
  if (db) {
    try {
      const { hospitalStaffProfiles, hospitals } = await import("../database/schema");
      const existing = await withDbTimeout(
