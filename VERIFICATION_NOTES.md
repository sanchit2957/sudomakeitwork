...
## Current CWC river-gauge fallback repair

The stale NWDP telemetry feed remains the preferred direct official source, but no longer prevents a current reading when it has no fresh observations. When it is stale or unavailable, the service retrieves the public Axom Flood feed only for observations explicitly identified as Central Water Commission Flood Forecasting Service data. It retains the matching official CWC station link, labels the relay in the UI, validates the expected schema and station proximity, and rejects any reading older than 48 hours. Browser verification at the default Guwahati map point displayed **47.32 m**, falling, at Guwahati D.C. Court, with a CWC-linked attribution. TypeScript passes and all **33** Vitest files / **77** tests pass.

## Comprehensive dark-theme contrast repair

The dark-mode audit found that Safety, More, and Tracking used route-local light surfaces that were outside the original Home-only override. Their pages now use shared Victim surface classes, which convert hard-coded light cards, borders, text, inputs, and controls to neutral charcoal and high-contrast foregrounds. Operations workspace cards, forms, sidebars, map overlays, protected-role helper content, map fallback, and shared dialog surfaces received matching dark treatment. Emergency red, warning amber, success green, and information blue remain visible as functional accents, while all large surfaces are black or charcoal. Browser checks confirmed readable persisted dark mode on Home, Safety, More, Tracking, Command, and Medical protected entry routes. TypeScript passes and all **34** Vitest files / **80** tests pass, including new dark-theme coverage regressions.

## Multilingual selector and current-interface repair

The language selector was initializing correctly but the current panic-first Victim and protected-access surfaces were primarily direct English strings not covered by the base dictionary or remote operational pack. The repair adds built-in translations for the current interface’s high-visibility controls, status labels, navigation, Tracking shell, and protected Command entry, merges those terms with the operational pack, and writes a user-selected locale to both browser storage and `?lang=` URL state. Hindi Home and persisted Hindi Tracking browser checks passed; Tamil Safety and Command checks also passed without layout issues. A browser-style regression proves that selecting Hindi changes visible copy, `document.documentElement.lang`, storage, and the URL. TypeScript passes and all **35** Vitest files / **82** tests pass.

## Safety tab development release

The Victim Safety tab now includes a device-local, three-step readiness plan; explicit location state and retry control; location-ordered verified shelters and medical resources; simplified, location-gated assistance requests; compact flood safety cards; and source-attributed official river-gauge status. The readiness checklist is deliberately local only and cannot create an operational request. The public dark-mode browser check rendered a high-risk weather state, fresh CWC-linked Guwahati gauge, available medical capacity, and all safety controls. Clicking the readiness item updated its state from **0/3** to **1/3 ready**. TypeScript passes and all **35** Vitest files / **84** tests pass.

## Hospital-resource Safety release

The general Safety assistance chooser was removed and replaced by a large nearby-hospital experience designed for low-technical-confidence users. It now prioritizes emergency actions, big bed/ICU tiles, four visual food/medicine/water/power indicators, last-updated context, and a direct verified contact action. A new sign-in-gated hospital registration route lets standard accounts submit a hospital for administrator review. Approval creates the hospital record, links the staff account, and grants Medical Operations access. Linked hospital staff can publish only live capacity and supply status for their own hospital; administrators retain hospital identity, location, total-capacity, and approval authority. The migration was applied and a read-only live query confirmed the new columns. TypeScript and all **36** Vitest files / **86** tests pass; public Safety and unauthenticated hospital registration routes were visually verified.

The Safety information order was subsequently simplified on request. **Verified medical care** now appears immediately below the emergency-call and Rapid SOS actions. **Local alert** follows the hospital section, and the device-only Safety Plan checklist has been removed. A 375 px full-page visual check confirmed the intended hierarchy and the absence of the checklist; TypeScript and all **36** Vitest files / **86** tests pass.

The nearby-hospital card now includes a compact, accessible Google Maps directions icon directly beside the hospital name. The link uses that verified record’s latitude and longitude as the Google Maps destination, opens in a separate tab with safe relationship attributes, and is announced with the hospital name. A loaded mobile browser check confirmed both its placement and a valid coordinate-based directions URL; TypeScript and all **36** Vitest files / **86** tests pass.

The Track flow now displays the active SOS public code in the existing live rescue-flow header as a clearly labelled **Tracking no.** badge. No other Track behavior or data flow was changed: request details and chat continue to receive the same public code. A focused regression protects the accessible label and the existing flow contract. The no-active-SOS mobile state was also visually rechecked and remains unchanged. TypeScript and all **37** Vitest files / **87** tests pass.

Assigned Rescuer location sharing is now automatic. Assignment sets the responder’s live-location state active, the Rescuer workspace requests and publishes the current browser position immediately and every **five seconds** while a mission remains active, and the matched user’s Track surfaces refresh on the same five-second cadence. The manual start/stop location control and its server mutation were removed. Location updates remain rescuer-only, require an open assigned mission, are exposed only to the matching SOS, and are cleared automatically on resolution. Profile and Track wording now explicitly explains automatic sharing rather than manual opt-in. TypeScript and all **39** Vitest files / **90** tests pass, including assignment-lifecycle, UI-control-removal, and five-second refresh regressions.

The active SOS user experience now adds a delivery-style live route. The matched status response carries the SOS’s existing coordinates only inside its already restricted assigned-rescuer payload. When fresh responder coordinates are present, the assigned-rescuer map renders two markers, a Google Maps driving route line, route distance, and live ETA; it recomputes as the responder location and five-second status refresh change. If Google Maps cannot return a route, the map remains usable and shows an explicit route-estimate-unavailable state rather than inventing an ETA. Routing support is loaded through the existing Maps proxy. TypeScript and all **41** Vitest files / **93** tests pass, including route privacy, directions, ETA, unavailable-state, proxy, and five-second refresh regressions.

The route display was then hardened after the ETA was visible but the directions-renderer overlay was not. The map now draws the driving result’s `overview_path` as an explicit high-contrast, seven-pixel red Google Maps polyline at a high z-index, while retaining the responder and SOS endpoint markers, route bounds, ETA, distance, and unavailable state. This removes reliance on the opaque renderer overlay and makes the actual rescuer-to-user line a direct map layer. TypeScript and all **41** Vitest files / **93** tests pass, including a regression that requires the visible polyline treatment.

For immediate endpoint recognition, the active route map now uses distinct Google Maps pin treatments: the SOS user’s location is a familiar blue map pin, and the assigned rescuer is a red pin marked `R`. Their descriptive map titles remain “Your SOS location” and “Assigned rescuer.” TypeScript and all **41** Vitest files / **93** tests pass, including direct marker-style and label regressions. A responsive Track-page capture confirmed the route did not introduce a layout or runtime regression; no live assigned SOS was present in the safe preview environment, so no operational incident data was created merely to capture map markers.
