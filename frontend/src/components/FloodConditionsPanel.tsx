...
function Stat({
  ...
  return (
    <div className="rounded-2xl bg-[#f8f7f2] p-3 dark:bg-[#202023]">
      <Icon className="h-5 w-5 text-[#881337] dark:text-[#facc15]" />
      <p className="mt-3 text-[11px] font-bold text-[#78716c] dark:text-[#a8a29e]">{label}</p>
      <p className="mt-0.5 text-lg font-black tracking-[-0.04em] text-[#1c1917] dark:text-[#f8fafc]">{value}</p>
      <p className="mt-0.5 text-[10px] font-semibold leading-4 text-[#78716c] dark:text-[#a8a29e]">
        {detail}
      </p>
    </div>
  );
...
export function FloodConditionsPanel({
  ...
  return (
    <section className="mt-5 rounded-[1.55rem] bg-white p-5 shadow-[0_12px_28px_rgba(22,60,53,.09)] ring-1 ring-black/[.035] dark:bg-[#1a1a1c] dark:ring-white/10">
      ...
      <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {onRefresh && (
              <button
                ...
                className="rounded-full p-1 text-[#78716c] hover:bg-[#fff1f2] hover:text-[#881337] active:scale-95 disabled:opacity-50 dark:hover:bg-[#252528] dark:hover:text-[#facc15]"
              >
                <RefreshCw className={`h-3.5 w-3.5 ${effectiveLoading ? "animate-spin text-[#881337] dark:text-[#facc15]" : ""}`} />
              </button>
            )}
          </div>
          <p className="mt-0.5 text-[11px] font-semibold text-[#78716c] dark:text-[#a8a29e]">
            {effectiveLoading
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          {effectiveConditions?.dataSource?.isCached && (
            <span className="rounded-md bg-[#f1f5f4] px-1.5 py-0.5 text-[9px] font-bold text-[#78716c] dark:bg-[#28282b] dark:text-[#a8a29e]">
              {t("Cached data")}
            </span>
          )}
        </div>
      ...
      <div className="mt-3 flex items-center justify-between gap-2 rounded-xl bg-[#f8f7f2] px-3 py-2 text-xs dark:bg-[#222225]">
        <button
          type="button"
          onClick={() => setShowLocationPicker((open) => !open)}
          className="flex items-center gap-1.5 text-left font-bold text-[#1c1917] hover:underline dark:text-[#f8fafc]"
        >
          <MapPin className="h-3.5 w-3.5 shrink-0 text-[#881337] dark:text-[#facc15]" />
        ...
        {onGpsLocate && (
          <button
            ...
            className={`flex shrink-0 items-center gap-1 rounded-lg px-2 py-1 text-[10px] font-bold transition ${
              isGpsActive
                ? "bg-[#881337] text-white dark:bg-[#eab308] dark:text-[#0a0b0d]"
                : "bg-white text-[#881337] shadow-sm hover:bg-[#fff1f2] dark:bg-[#2d2d30] dark:text-[#facc15]"
            }`}
          >
      ...
      {showLocationPicker && (
        <div className="mt-2 rounded-xl border border-black/10 bg-white p-2.5 dark:border-white/10 dark:bg-[#1e1e21]">
          <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-[#78716c] dark:text-[#a8a29e]">
            {t("Select Location:")}
          </p>
          <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">
            {POPULAR_ASSAM_LOCATIONS.map((loc) => (
              <button
                ...
                className="rounded-lg bg-black/5 p-1.5 text-left text-[11px] font-semibold text-[#1c1917] shadow-sm transition hover:bg-[#881337] hover:text-white dark:bg-[#29292c] dark:text-[#f8fafc] dark:hover:bg-[#eab308] dark:hover:text-[#0a0b0d]"
              >
                {loc.name}
              </button>
            ))}
      ...
      {effectiveConditions?.current?.condition && (
        <div className="mt-3 flex items-center justify-between gap-2 rounded-xl bg-[#fff1f2] px-3.5 py-2.5 text-xs font-bold text-[#881337] dark:bg-[#450a0a]/30 dark:text-[#fca5a5]">
          <div className="flex items-center gap-2">
            <WeatherIcon className="h-4 w-4 shrink-0 text-[#881337] dark:text-[#facc15]" />
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <button
          ...
          className="rounded-2xl bg-[#f8f7f2] p-3 text-left transition active:scale-[.98] dark:bg-[#202023]"
        >
          <TrendingUp className="h-5 w-5 text-[#881337] dark:text-[#facc15]" />
          <p className="mt-3 text-[11px] font-bold text-[#78716c] dark:text-[#a8a29e]">
            {t("7-day trend")}
          </p>
          <p className="mt-0.5 text-[10px] font-semibold text-[#881337] dark:text-[#facc15]">
      ...
      {hourlyItems.length > 0 && (
        <section className="mt-4 border-t border-black/10 pt-3 dark:border-white/10">
          <div className="flex items-center gap-2">
            <Clock className="h-4 w-4 text-[#881337] dark:text-[#facc15]" />
            <h3 className="text-xs font-black text-[#1c1917] dark:text-[#f8fafc]">
          ...
          <div className="mt-2.5 flex gap-2 overflow-x-auto pb-1 scrollbar-thin">
            {hourlyItems.slice(0, 12).map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex min-w-[62px] shrink-0 flex-col items-center rounded-xl bg-[#f8f7f2] p-2 text-center dark:bg-[#202023]"
                >
                  <span className="text-[10px] font-bold text-[#78716c] dark:text-[#a8a29e]">
                    {formatHour(item.time)}
                  </span>
                  <HIcon className="my-1.5 h-4 w-4 text-[#881337] dark:text-[#facc15]" />
                  <span className="text-xs font-black text-[#1c1917] dark:text-[#f8fafc]">
                    {item.temperatureC != null ? `${Math.round(item.temperatureC)}°` : "—"}
                  </span>
                  <span className="mt-0.5 text-[9px] font-semibold text-[#881337] dark:text-[#facc15]">
      ...
      {effectiveConditions?.airQuality?.aqiUs !== null && effectiveConditions?.airQuality?.aqiUs !== undefined && (
        <div className="mt-3 flex items-center justify-between rounded-xl border border-black/10 bg-[#f8f7f2] px-3.5 py-2 text-xs font-semibold dark:border-white/10 dark:bg-[#202023]">
          <span className="flex items-center gap-2 text-[#78716c] dark:text-[#a8a29e]">
            <Compass className="h-4 w-4 text-[#881337] dark:text-[#facc15]" />
            {t("Air Quality:")} <strong className="text-[#1c1917] dark:text-[#f8fafc]">{effectiveConditions.airQuality.category}</strong> (AQI {effectiveConditions.airQuality.aqiUs})
          </span>
      ...
      <section className="mt-4 border-t border-black/10 pt-4 dark:border-white/10">
        <div className="flex items-center gap-2">
          <CalendarDays className="h-4 w-4 text-[#881337] dark:text-[#facc15]" />
          <h3 className="text-sm font-black text-[#1c1917] dark:text-[#f8fafc]">
            {t("Weather forecast")}
          </h3>
        </div>
          ...
          {forecastDays.length ? (
            forecastDays.map((day) => {
              ...
              return (
                <div
                  key={day.date}
                  className="rounded-xl bg-[#f8f7f2] px-1 py-2 text-center dark:bg-[#202023]"
                >
                  <span className="block text-[9px] font-black text-[#78716c] dark:text-[#a8a29e]">
                    {dayName(day.date)}
                  </span>
                  <DayIcon className="mx-auto my-1 h-3.5 w-3.5 text-[#881337] dark:text-[#facc15]" />
                  <span className="block text-[10px] font-black text-[#1c1917] dark:text-[#f8fafc]">
                    {day.rainChance ?? "—"}%
                  </span>
                  <span className="mt-0.5 block text-[9px] font-semibold text-[#78716c] dark:text-[#a8a29e]">
                    {day.rainMm ?? "—"} mm
                  </span>
                </div>
              );
            ...
            <p className="col-span-7 text-xs text-[#78716c] dark:text-[#a8a29e]">
      ...
      <div className="mt-3 rounded-2xl bg-[#fff1f2] px-3 py-3 text-xs font-bold text-[#881337] dark:bg-[#450a0a]/30 dark:text-[#fca5a5]">
        <div className="flex items-center gap-3">
          <Waves className="h-5 w-5 shrink-0 text-[#881337] dark:text-[#facc15]" />
          <span>
        ...
        {effectiveConditions?.river?.sourceUrl && effectiveConditions?.river?.sourceName ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="ml-8 mt-1 inline-block text-[10px] font-bold text-[#881337] underline underline-offset-2 dark:text-[#facc15]"
          >
