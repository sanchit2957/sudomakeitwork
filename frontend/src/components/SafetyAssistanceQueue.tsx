...
export function SafetyAssistanceQueue({
  ...
  return (
    <section className={`rounded-3xl border bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#16171a] ${compact ? "" : "space-y-5"}`}>
      ...
      {guidance?.length ? (
        <div className="mt-4 rounded-2xl border border-[#881337]/20 bg-[#fff1f2]/50 p-4 dark:border-[#eab308]/20 dark:bg-[#422006]/20">
          <p className="text-xs font-extrabold text-[#881337] dark:text-[#facc15]">Before you acknowledge</p>
                ...
                <div className="flex flex-col justify-between gap-4 sm:flex-row">
                  <div className="min-w-0 space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="grid h-8 w-8 place-items-center rounded-xl bg-[#fff1f2] text-[#881337] dark:bg-[#881337]/20 dark:text-[#fca5a5]">
                        <Icon className="h-4 w-4" />
                      </span>
                      <span className="text-sm font-extrabold capitalize text-foreground">{item.category} support</span>
...
function SafetyStatus({ value }: { value: string }) {
  const classes =
    value === "new"
      ...
      : "bg-[#fefce8] text-[#854d0e] dark:bg-[#422006]/50 dark:text-[#fef08a]";
