...
export function HospitalStaffDashboard({ hospital }: { hospital: HospitalRecord | null | undefined }) {
  ...
  const readiness = useMemo(() => {
    ...
    return { level: "READY", color: "bg-[#881337]", text: "text-[#881337] dark:text-[#facc15]", border: "border-[#881337]/40 dark:border-[#facc15]/40", label: "OPERATIONAL & READY" };
  }, [hospital, draft]);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-3xl border border-border/80 bg-gradient-to-br from-[#4c0519] via-[#881337] to-[#9f1239] p-6 text-white shadow-xl">
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
          <div className="flex items-start gap-4">
            <span className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-white/10 shadow-inner backdrop-blur-md">
              <Hospital className="h-7 w-7 text-[#fde047]" />
            </span>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-mono text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#fef08a]">
                  Emergency Hospital Operations Desk
                </span>
                <span className="rounded-md bg-white/15 px-2 py-0.5 font-mono text-[9px] font-black uppercase text-[#fef08a]">
                  Verified Facility #{hospital.id}
                </span>
              </div>
              <h1 className="mt-1 text-2xl font-black tracking-tight sm:text-3xl">{hospital.name}</h1>
              <p className="mt-1 flex flex-wrap items-center gap-3 text-xs text-[#fecdd3]">
      ...
      {activeTab === "resources" && (
        <section className="space-y-6">
          ...
          {savedNotice && (
            <div className="flex items-center gap-2 rounded-2xl border border-[#ca8a04]/30 bg-[#fefce8] p-4 text-xs font-bold text-[#854d0e] dark:border-[#eab308]/30 dark:bg-[#422006]/30 dark:text-[#fef08a]">
              <CheckCircle2 className="h-4 w-4 shrink-0 text-[#ca8a04] dark:text-[#facc15]" />
...
function QuickCapacityCard({
  ...
  const tones = {
    green: "bg-[#fff1f2] text-[#881337] dark:bg-[#450a0a]/40 dark:text-[#fca5a5]",
    blue: "bg-blue-500/10 text-blue-600 dark:text-blue-400",
    emerald: "bg-[#fef9c3] text-[#854d0e] dark:bg-[#451a03]/40 dark:text-[#fde047]",
      ...
      {total !== undefined && (
        <div className="mt-3">
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
            <div
              className={`h-full ${pct && pct < 20 ? "bg-red-500" : pct && pct < 40 ? "bg-amber-500" : "bg-[#881337] dark:bg-[#facc15]"}`}
              style={{ width: `${pct ?? 0}%` }}
            />
          </div>
...
function CaseCard({
  ...
  const statusMap: Record<string, { label: string; tone: string }> = {
    ...
    ready: { label: "READY FOR ARRIVAL", tone: "bg-[#fff1f2] text-[#881337] border-[#881337]/30 dark:bg-[#450a0a]/30 dark:text-[#fecdd3]" },
    received: { label: "PATIENTS RECEIVED", tone: "bg-[#fef9c3] text-[#854d0e] border-[#facc15]/30 dark:bg-[#451a03]/40 dark:text-[#fde047]" },
          ...
          {(notification.icuRequired === "yes" || notification.oxygenRequired === "yes") && (
            <div className="flex gap-2 pt-1">
              ...
              {notification.oxygenRequired === "yes" && (
                <span className="rounded-md bg-[#fef9c3] px-2 py-0.5 font-mono text-[10px] font-bold text-[#854d0e] dark:bg-[#451a03]/40 dark:text-[#fde047]">
        ...
        <div className="flex flex-wrap gap-2 sm:flex-col sm:items-end">
          ...
          {notification.status === "preparing" && (
            <Button
              ...
              className="rounded-xl bg-[#881337] text-xs font-bold text-white hover:bg-[#9f1239]"
            >
          ...
          {notification.status === "ready" && (
            <Button
              ...
              className="rounded-xl bg-[#ca8a04] text-xs font-bold text-white hover:bg-[#a16207]"
            >
