...
import { buildEmergencySmsUri, ASSAM_EMERGENCY_NUMBERS } from "@/lib/emergencyDispatch";
...
import { AlertTriangle, Bluetooth, CloudRain, Database, Hospital, MapPin, MessageSquare, MoreHorizontal, Navigation, PhoneCall, Radio, RefreshCw, ShieldCheck, Siren, TentTree, ThermometerSun, Waves, Wifi, WifiOff } from "lucide-react";
...
export default function Home() {
  ...
  const handleGpsLocate = async () => {
    ...
    try {
      ...
      console.warn("[Home] GPS locate failed:", err);
      if (!position) {
  ...
  useEffect(() => {
    ...
    // Fast non-blocking location acquisition
    void getCurrentCoordinates({ enableHighAccuracy: true, timeout: 5000 })
      ...
      .catch((err) => {
        console.warn("[Home] Initial location acquisition note:", err);
  ...
  const startRapidSos = () => {
    ...
    const submitSosWithCoords = async (latitude: number, longitude: number, label: string) => {
      ...
      setRapidNotice(t("Transmitting priority emergency dispatch request…"));

      try {
        const created = await createSos.mutateAsync(payload);
        clearSosVoiceNote();
        setRapidStatus("idle");
        redirectAfterRapidSos(created.publicCode, setLocation);
      } catch (error) {
    ...
    if (typeof navigator !== "undefined" && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          const fallback = position || { latitude: INDIA_CENTER.lat, longitude: INDIA_CENTER.lng };
          submitSosWithCoords(fallback.latitude, fallback.longitude, t("National emergency coordinates"));
        },
  ...
  return (
    <div className="victim-page min-h-screen bg-[#f6f8f7] text-[#142c2b] dark:bg-[#050505] dark:text-[#f4f4f5]">
      <main className="victim-main relative mx-auto min-h-screen max-w-lg overflow-hidden bg-[#fcfdfd] px-5 pb-28 pt-6 shadow-2xl shadow-[#113c35]/10 dark:bg-[#101011] dark:shadow-black/30 md:my-6 md:min-h-[850px] md:rounded-[2.75rem] md:border">
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            <span
              className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-extrabold ${
                online
                  ? "bg-[#fef9c3] text-[#854d0e] border border-[#facc15]/30 dark:bg-[#422006]/60 dark:text-[#fde047]"
                  : "bg-[#fff1f2] text-[#881337] border border-[#881337]/30 dark:bg-[#450a0a]/60 dark:text-[#fca5a5]"
              }`}
            >
        ...
        <section className="mt-4 rounded-[1.55rem] bg-white p-4 shadow-[0_12px_28px_rgba(22,60,53,.07)] ring-1 ring-black/[.035] dark:bg-[#18191d] dark:ring-white/10">
          <div className="flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-sm font-black text-[#1c1917] dark:text-[#f8fafc]">
              <Hospital className="h-4 w-4 text-[#881337] dark:text-[#facc15]" />
              <span>{t("Nearby Emergency Facilities")}</span>
            </h3>
            <span className="rounded-full bg-[#881337]/10 px-2 py-0.5 text-[10px] font-black text-[#881337] dark:bg-[#eab308]/20 dark:text-[#facc15]">
          ...
          {nearbyHospitals.length > 0 || nearbyShelters.length > 0 ? (
            <div className="mt-3 space-y-2">
              {nearbyHospitals.slice(0, 3).map((h) => (
                <div
                  key={`hosp-${h.id}`}
                  className="flex items-center justify-between rounded-xl border border-[#881337]/20 bg-[#fff1f2]/50 p-2.5 text-xs dark:border-white/10 dark:bg-[#1a1c20]"
                >
                  <div>
                    <div className="font-bold text-[#1c1917] dark:text-[#f8fafc]">{h.name}</div>
                  ...
                  {h.contactPhone && (
                    <a
                      ...
                      className="rounded-lg bg-[#881337] px-2.5 py-1 text-[11px] font-bold text-white shadow-sm hover:bg-[#9f1239] active:scale-95 dark:bg-[#eab308] dark:text-[#0a0b0d] dark:hover:bg-[#facc15]"
                    >
                      {t("Call")}
                    </a>
                  )}
...
export function RapidSosControl({
  authenticated,
  authLoading,
  status,
  notice,
  ...
  authLoading: boolean;
  status: "idle" | "locating" | "sending" | "queued" | "error";
  notice: string;
  onActivate: () => void;
}) {
  const { t } = useLanguage();
  return (
    <section className="mt-6 flex flex-col items-center">
      <button
        onClick={onActivate}
        disabled={authLoading || status === "locating" || status === "sending"}
        aria-label={t("Send SOS")}
        className="group isolate grid h-44 w-44 shrink-0 aspect-square place-items-center overflow-hidden rounded-[9999px] border border-white/55 bg-[linear-gradient(145deg,rgba(255,109,118,.91),rgba(209,47,55,.84)_55%,rgba(174,27,36,.9))] text-white ring-1 ring-[#ca3540]/25 backdrop-blur-md transition active:scale-[.975] disabled:cursor-wait disabled:opacity-80"
      >
        <span aria-hidden="true" className="pointer-events-none absolute inset-x-5 top-3 h-16 rounded-full bg-white/25 blur-md" />
      ...
      {notice && (
        <p
          role="status"
          className={`mt-3 max-w-xs text-center text-xs font-semibold leading-5 ${
            status === "error" ? "text-[#b73f43]" : "text-[#38675d]"
          }`}
        >
          {notice}
        </p>
...
function Stat({ icon: Icon, label, value, detail }: { icon: typeof CloudRain; label: string; value: string; detail: string }) {
  return (
    <div className="rounded-2xl bg-[#f8f7f2] p-3 dark:bg-[#1a1c20]">
      ...
      <p className="mt-0.5 text-xl font-black tracking-[-0.04em] text-[#1c1917] dark:text-[#f8fafc]">{value}</p>
      <p className="mt-0.5 text-[10px] font-semibold text-[#a8a29e] dark:text-[#78716c]">{detail}</p>
    </div>
  );
}

export function VictimNavigation({ current }: { current: "home" | "track" | "safety" | "more" }) {
  const [, setLocation] = useLocation();
  const { t } = useLanguage();
  const items = [
    { id: "home" as const, icon: Siren, label: t("SOS"), path: "/" },
    { id: "track" as const, icon: Navigation, label: t("Track"), path: "/track" },
    { id: "safety" as const, icon: ShieldCheck, emoji: "🛡️", label: t("Safety"), path: "/safety" },
    { id: "more" as const, icon: MoreHorizontal, emoji: "⋯", label: t("More"), path: "/more" },
  ];
  return (
    <nav
      aria-label="Victim App navigation"
      className="fixed inset-x-0 bottom-0 z-40 mx-auto flex max-w-lg justify-around border-t border-black/10 bg-[#fdfcf7]/95 px-4 pb-[max(0.7rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur transition-colors dark:border-white/10 dark:bg-[#121316]/95 md:bottom-6 md:rounded-[2rem] md:border md:shadow-2xl"
    >
      {items.map(({ id, icon: Icon, emoji, label, path }) => (
        <button
          key={id}
          onClick={() => setLocation(path)}
          className={`group grid min-w-16 place-items-center gap-1 rounded-2xl px-3 py-1.5 text-[10px] font-black transition-all active:scale-95 ${
            current === id
              ? "text-[#881337] dark:text-[#facc15]"
              : "text-[#78716c] hover:text-[#1c1917] dark:text-[#a8a29e] dark:hover:text-[#f8fafc]"
          }`}
        >
          <span
            className={`grid h-8 w-8 place-items-center rounded-xl transition-all ${
              current === id
                ? "bg-[#881337]/10 shadow-sm dark:bg-[#eab308]/20"
