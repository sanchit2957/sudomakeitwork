...
export default function UserHome() {
  ...
  return (
    <div className="victim-page min-h-screen bg-[#f8f7f2] text-[#1c1917] transition-colors dark:bg-[#0a0b0d] dark:text-[#f8fafc]">
      <main className="victim-main relative mx-auto min-h-screen max-w-lg overflow-hidden bg-[#fdfcf7] px-5 pb-28 pt-6 shadow-2xl shadow-black/10 transition-colors dark:bg-[#121316] dark:shadow-black/60 md:my-6 md:min-h-[850px] md:rounded-[2.75rem] md:border md:border-black/10 dark:md:border-white/10">
...
function FloodConditions({ conditions, loading }: { conditions?: { available: boolean; risk: string; activeFloodZones: number; current: { temperatureC: number | null; precipitationMm: number | null; windKmh: number | null }; forecast: { rainChance: number | null; rainAmountMm: number | null } }; loading: boolean }) {
  ...
  return (
    <section className="mt-5 rounded-[1.55rem] bg-white p-5 shadow-[0_12px_28px_rgba(22,60,53,.09)] ring-1 ring-black/[.035]">
      ...
      <div className="mt-3 flex items-center gap-3 rounded-2xl bg-[#fff1f2] px-3 py-3 text-xs font-bold text-[#881337] dark:bg-[#450a0a]/30 dark:text-[#fca5a5]">
...
function Stat({ icon: Icon, label, value, detail }: { icon: typeof CloudRain; label: string; value: string; detail: string }) { return <div className="rounded-2xl bg-[#f8f7f2] p-3 dark:bg-[#1a1c20]"><Icon className="h-5 w-5 text-[#881337] dark:text-[#facc15]" /><p className="mt-3 text-[11px] font-bold text-[#78716c] dark:text-[#a8a29e]">{label}</p><p className="mt-0.5 text-xl font-black tracking-[-0.04em] text-[#1c1917] dark:text-[#f8fafc]">{value}</p><p className="mt-0.5 text-[10px] font-semibold text-[#a8a29e] dark:text-[#78716c]">{detail}</p></div>; }

export function VictimNavigation({ current }: { current: "home" | "track" | "safety" | "more" }) {
  const [, setLocation] = useLocation();
  const { t } = useLanguage();
  const items = [
    { id: "home" as const, icon: Siren, emoji: "🚨", label: t("SOS"), path: "/" },
    { id: "track" as const, icon: Navigation, emoji: "📍", label: t("Track"), path: "/track" },
