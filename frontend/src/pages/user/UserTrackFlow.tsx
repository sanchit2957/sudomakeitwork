...
import { useLiveRescuerStream } from "@/hooks/useLiveRescuerStream";
...
export default function UserTrackFlow() {
  ...
  const liveStream = useLiveRescuerStream(/^SOS-[A-Z0-9]{8}$/.test(publicCode) ? publicCode : null);
  const status = trpc.rescue.emergency.statusByCode.useQuery({ publicCode }, { enabled: /^SOS-[A-Z0-9]{8}$/.test(publicCode), refetchInterval: liveStream.pollingIntervalMs ?? 20_000, refetchIntervalInBackground: true, retry: false });
          ...
          <section className="mt-5 overflow-hidden rounded-[2rem] border border-black/10 bg-white shadow-xl dark:border-white/10 dark:bg-[#18191d]">
            ...
            <div className="bg-gradient-to-br from-[#121316] via-[#1a1c22] to-[#0a0b0d] p-5 text-white dark:from-[#0a0b0d] dark:via-[#16181d] dark:to-[#0f1013]">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                <p aria-label={t("SOS tracking number")} className="inline-flex items-center gap-2 rounded-xl bg-white/15 px-3 py-1.5 font-mono text-xs font-black tracking-wide text-white ring-1 ring-white/20">
            ...
            <div className="p-5">
              ...
              {status.data.assignedRescuer && (
                <div className="mt-5 rounded-2xl border border-black/5 bg-[#fdfcf7] p-3 shadow-sm dark:border-white/5 dark:bg-[#121316]">
                  <AssignedRescuerCard
                    rescuer={status.data.assignedRescuer}
                    hospital={(status.data as any).destinationHospital}
                    liveRescuer={liveStream.rescuer}
                    liveRoute={liveStream.route}
                    isStale={liveStream.isStale}
