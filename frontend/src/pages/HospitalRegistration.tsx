...
export default function HospitalRegistration() {
  ...
  return <div className="min-h-screen app-grid bg-[#fcfdfd] dark:bg-[#0c0a09] p-4 sm:p-8"><div className="absolute right-4 top-4"><LanguageSelector compact /></div><main className="mx-auto w-full max-w-2xl rounded-[2rem] border bg-card p-5 shadow-xl sm:p-8"><div className="flex items-start gap-4"><span className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-[#fff1f2] text-[#881337] dark:bg-[#450a0a]/40 dark:text-[#fde047]"><Building2 className="h-7 w-7" /></span><div><p className="font-mono text-[10px] font-bold uppercase tracking-[.18em] text-[#881337] dark:text-[#facc15]">Verified hospital network</p><h1 className="mt-1 text-2xl font-extrabold">Register your hospital</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">Share the hospital location and a contact. After approval, staff can update beds, ICU capacity, transport, food, medicine, water, and power status in real time.</p></div></div><div className="mt-7 grid gap-4"><FormField label="Hospital or health centre name" value={form.hospitalName} onChange={hospitalName => setForm({ ...form, hospitalName })} /><FormField label="Address or landmark" value={form.address} onChange={address => setForm({ ...form, address })} /><FormField label="Emergency contact number" value={form.contactPhone} onChange={contactPhone => setForm({ ...form, contactPhone })} type="tel" /><div className="rounded-2xl border border-[#fecdd3] bg-[#fff1f2]/30 dark:border-[#881337]/40 dark:bg-[#450a0a]/20 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-extrabold">Hospital location</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Use the device location at the hospital, or enter coordinates below.</p></div><Button type="button" variant="outline" onClick={useCurrentLocation} className="rounded-xl"><MapPin className="mr-2 h-4 w-4" />Use location</Button></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><FormField label="Latitude" value={form.latitude} onChange={latitude => setForm({ ...form, latitude })} type="number" /><FormField label="Longitude" value={form.longitude} onChange={longitude => setForm({ ...form, longitude })} type="number" /></div>{locationNotice && <p className="mt-3 text-xs font-semibold text-[#881337] dark:text-[#facc15]">{locationNotice}</p>}</div><div className="grid gap-2"><Label>Verification note (optional)</Label><Textarea value={form.note} onChange={event => setForm({ ...form, note: event.target.value })} className="min-h-24" /></div></div><Button disabled={!form.hospitalName.trim() || !form.address.trim() || !form.contactPhone.trim() || request.isPending} onClick={submit} className="mt-6 h-12 w-full rounded-xl bg-[#881337] hover:bg-[#9f1239] text-white"><ClipboardCheck className="mr-2 h-4 w-4" />{request.isPending ? "Submitting registration…" : "Send for administrator approval"}</Button>{request.isSuccess && <p className="mt-4 rounded-xl bg-[#fff1f2] p-3 text-sm font-semibold text-[#881337] dark:bg-[#450a0a]/30 dark:text-[#fde047]">Registration sent. You will receive hospital dashboard access after approval.</p>}{request.error && <p className="mt-4 text-sm font-semibold text-destructive">{request.error.message}</p>}</main></div>;
}
