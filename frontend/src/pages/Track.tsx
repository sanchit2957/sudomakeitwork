...
export default function Track() {
  ...
  const stages = useMemo(() => [
    ...
    { key: "dispatched", short: t("Rescuer assigned"), label: t("Rescuer assigned"), icon: Radio, tone: "bg-[#fff1f2] text-[#881337]" },
    { key: "resolved", short: t("Rescue completed"), label: t("Rescue completed"), icon: CheckCircle2, tone: "bg-[#fef9c3] text-[#854d0e]" },
  ] as const, [t]);
  useEffect(() => {
  ...
  return <div className="min-h-screen bg-[#fcfdfd] dark:bg-[#0c0a09]"><main className="mx-auto min-h-screen max-w-lg bg-[#fcfdfd] px-5 pb-28 pt-6 md:my-6 md:min-h-[850px] md:rounded-[2.75rem] md:border dark:bg-[#141211]"><header className="flex items-start justify-between"><button onClick={() => setLocation("/")} className="flex items-center gap-2 text-left"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#881337] text-white"><ArrowLeft className="h-5 w-5" /></span><span><span className="block text-xl font-black tracking-[-.05em]">{t("track.heading")}</span><span className="block text-[10px] font-bold text-muted-foreground">{t("track.private")}</span></span></button><LanguageSelector compact /></header><form onSubmit={search} className="mt-7 flex gap-2 rounded-[1.35rem] bg-white p-2 shadow-[0_12px_28px_rgba(136,19,55,.09)] ring-1 ring-black/[.035] dark:bg-[#1c1917]"><Input value={code} onChange={event => setCode(event.target.value.toUpperCase())} placeholder="SOS-XXXXXXXX" aria-label={t("track.code")} className="h-11 border-0 font-mono font-medium shadow-none focus-visible:ring-0" /><Button type="submit" className="h-11 rounded-xl bg-[#881337] hover:bg-[#9f1239] text-white"><Search className="h-4 w-4" /></Button></form>{status.isFetching && <p className="mt-4 flex items-center justify-center gap-2 text-xs text-muted-foreground"><Clock3 className="h-3.5 w-3.5 animate-spin" /> {t("track.checking")}</p>}{status.error && <div role="alert" className="mt-5 flex gap-3 rounded-2xl border border-[#f3c4c1] bg-[#fff5f4] p-4 text-sm text-[#a53d38] dark:bg-[#450a0a]/30"><AlertCircle className="mt-0.5 h-5 w-5 shrink-0" /><p>{status.error.message}</p></div>}{status.data && <section className="mt-5 overflow-hidden rounded-[1.8rem] bg-white shadow-[0_18px_40px_-28px_rgb(136_19_55/0.35)] ring-1 ring-black/[.035] dark:bg-[#1c1917]"><div className="bg-[#881337] p-5 text-white"><div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#fef08a]">{t("track.yourSos")}</p><p className="mt-1 font-mono text-lg font-black">{status.data.publicCode}</p></div><div className="flex flex-col items-end gap-1"><span className="rounded-full bg-white/10 px-3 py-1 text-xs font-extrabold">{t("track.liveCheck")}</span>{(status.data as any).requestCategory && <span className="rounded-full bg-[#facc15]/20 px-2.5 py-0.5 text-[10px] font-black uppercase text-[#fde047] ring-1 ring-[#facc15]/30">{(status.data as any).requestCategory}</span>}</div></div><div className="mt-5 flex items-center gap-4"><div className="grid h-16 w-16 shrink-0 place-items-center rounded-2xl bg-white/15 backdrop-blur-sm"><CurrentIcon className="h-8 w-8 text-white" /></div><div><div className="flex items-center gap-2 text-[#fef08a]"><CurrentIcon className="h-4 w-4" /><span className="text-xs font-bold uppercase tracking-wider">{t("track.now")}</span></div><p className="mt-2 text-lg font-extrabold leading-tight">{dispatchStatusLabel}</p></div></div></div><div className="p-4"><div className="grid grid-cols-3 gap-2">{stages.map((stage, index) => { const Icon = stage.icon; const complete = index <= activeIndex; return <div key={stage.key} className={`rounded-2xl p-3 text-center ${complete ? stage.tone : "bg-muted text-muted-foreground"}`}><Icon className="mx-auto h-5 w-5" /><span className="mt-2 block text-xs font-extrabold">{stage.short}</span></div>; })}</div>{(status.data as any).dispatchStatus === "escalated" && <div role="status" className="mt-4 flex items-center gap-2.5 rounded-2xl border border-amber-500/40 bg-amber-500/10 p-3 text-xs font-bold text-amber-900 dark:text-amber-300"><Clock3 className="h-5 w-5 shrink-0 text-amber-600" /><span>{t("State Command Centre has taken direct priority oversight of this incident.")}</span></div>}{(status.data as any).destinationHospitalName && <div className="mt-4 flex items-center gap-2.5 rounded-2xl border border-[#881337]/30 bg-[#fff1f2] dark:border-[#881337]/50 dark:bg-[#450a0a]/30 p-3.5 text-xs font-extrabold text-[#881337] dark:text-[#fecdd3]"><span>🏥</span><span>{t("En route to")} {(status.data as any).destinationHospitalName}</span></div>}<div className="mt-4 flex items-center gap-3 rounded-2xl bg-[#fff1f2] p-3 text-sm font-semibold text-[#881337] dark:bg-[#450a0a]/30 dark:text-[#fecdd3]"><MapPin className="h-5 w-5 shrink-0 text-[#881337] dark:text-[#facc15]" /><span>{status.data.locationLabel}</span></div>{status.data.assignedRescuer && <AssignedRescuerCard rescuer={status.data.assignedRescuer} hospital={(status.data as any).destinationHospital} liveRescuer={liveStream.rescuer} liveRoute={liveStream.route} isStale={liveStream.isStale} lastEventAt={liveStream.lastEventAt} />}{status.data.status === "resolved" && <PostRescueCheckInCard publicCode={status.data.publicCode} />}<IncidentChat publicCode={status.data.publicCode} active={status.data.status !== "resolved"} /><div className="mt-4 rounded-xl border border-[#fecdd3] bg-[#fff1f2]/50 p-3 text-xs leading-5 text-[#881337] dark:border-[#881337]/40 dark:bg-[#450a0a]/20 dark:text-[#fecdd3]"><ShieldCheck className="mr-2 inline h-4 w-4 text-[#881337] dark:text-[#facc15]" /> {t("track.safetyNote")}</div></div></section>}</main><VictimNavigation current="track" /></div>;
}
...
export function IncidentChat({ publicCode, active }: { publicCode: string; active: boolean }) {
  ...
  return <section className="mt-4 rounded-2xl border border-[#fecdd3] bg-card p-3"><div className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-xl bg-[#fff1f2] text-[#881337] dark:bg-[#450a0a]/40 dark:text-[#fde047]"><MessageCircle className="h-4 w-4" /></span><div><h2 className="text-sm font-black">{t("Message your rescue team")}</h2><p className="text-[10px] font-semibold text-muted-foreground">{active ? t("Use short, useful updates") : t("This SOS is resolved")}</p></div></div><div className="mt-3 max-h-36 space-y-2 overflow-y-auto pr-1">{messages.data?.length ? messages.data.map(item => <div key={item.id} className={`max-w-[88%] rounded-2xl px-3 py-2 text-xs leading-5 ${item.authorType === "victim" ? "ml-auto bg-[#881337] text-white" : "bg-[#fff1f2] text-[#881337] dark:bg-[#450a0a]/30 dark:text-[#fecdd3]"}`}><span className="block text-[9px] font-black uppercase tracking-wide opacity-70">{item.authorType === "rescuer" ? t("Rescuer") : item.authorType === "operations" ? t("Operations") : t("You")}</span>{item.message}</div>) : <p className="rounded-xl bg-muted px-3 py-2 text-xs text-muted-foreground">{t("Your assigned rescue team will see messages here.")}</p>}</div>{active && (user ? <div className="mt-3 flex gap-2"><Input value={message} onChange={event => setMessage(event.target.value)} maxLength={500} placeholder={t("Type a short update")} className="h-10 bg-white text-xs dark:bg-[#1c1917]" /><Button disabled={!message.trim() || send.isPending} onClick={() => send.mutate({ publicCode, message: message.trim() })} className="h-10 w-10 shrink-0 rounded-xl bg-[#881337] hover:bg-[#9f1239] text-white p-0"><Send className="h-4 w-4" /></Button></div> : <button onClick={() => startLogin()} className="mt-3 w-full rounded-xl bg-[#881337] hover:bg-[#9f1239] px-3 py-2.5 text-xs font-black text-white">{t("Sign in to send a message")}</button>)}{send.error && <p className="mt-2 text-[10px] font-bold text-destructive">{send.error.message}</p>}</section>;
}
...
export function AssignedRescuerCard({
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-2xl border border-[#fecdd3] bg-card dark:border-[#881337]/40">
      <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          <div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-2xl bg-[#fef9c3] text-[#854d0e] dark:bg-[#451a03]/40 dark:text-[#fde047]">
            {rescuer.photoUrl ? <img src={rescuer.photoUrl} alt="" className="h-full w-full object-cover" /> : <UserRound className="h-6 w-6" />}
          </div>
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[#881337] dark:text-[#facc15]">{t("Your assigned rescuer")}</p>
            <h2 className="mt-1 text-lg font-extrabold text-foreground">{rescuer.callSign}</h2>
            {rescuer.name && <p className="text-sm text-muted-foreground">{rescuer.name}</p>}
          </div>
        </div>
        {rescuer.phone ? (
          <a href={`tel:${rescuer.phone}`} className="inline-flex h-10 items-center justify-center rounded-xl bg-[#881337] hover:bg-[#9f1239] px-4 text-sm font-extrabold text-white">
            <Phone className="mr-2 h-4 w-4" /> {t("Call rescuer")}
          </a>
        ) : (
          <span className="rounded-xl border bg-white px-3 py-2 text-xs font-semibold text-muted-foreground dark:bg-[#1c1917]">
      ...
      {effectiveRoute && (
        <div className={`border-t border-[#fecdd3] p-4 dark:border-[#881337]/30 ${isArriving ? "bg-[#fef9c3]/40 dark:bg-[#451a03]/30" : "bg-[#fff1f2]/40 dark:bg-[#450a0a]/20"}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-2xl font-black text-[#881337] dark:text-[#fde047]">
                {isArriving ? "🏁 Arriving now" : `${effectiveRoute.isApproximate ? "~" : ""}${effectiveRoute.etaText}`}
              </p>
              <p className="mt-0.5 text-sm font-semibold text-muted-foreground">{effectiveRoute.distanceText} away{effectiveRoute.isApproximate ? " (estimated)" : ""}</p>
            </div>
            <div className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wide ${
              isStale || secondsAgo > LIVE_LOCATION_STALE_THRESHOLD_MS / 1000
                ...
                ? "bg-[#fef9c3] text-[#854d0e] dark:bg-[#451a03]/50 dark:text-[#fde047]"
      ...
      {hospital && (
        <div className="border-t border-[#fecdd3] bg-[#fff1f2]/60 dark:bg-[#450a0a]/30 p-3.5 text-xs text-[#881337] dark:text-[#fecdd3]">
          <div className="flex items-center gap-2 font-extrabold text-sm text-[#881337] dark:text-[#facc15]">
            <span>🏥</span> <span>Destination Hospital: {hospital.name}</span>
          </div>
          <p className="mt-1 text-[#9f1239] dark:text-[#fde047]">📍 {hospital.address} · Specialty: {hospital.specialty}</p>
...
export function PostRescueCheckInCard({ publicCode }: { publicCode: string }) {
  ...
  if (existingCheckIn.data) {
    ...
    return (
      <section className="mt-6 overflow-hidden rounded-2xl border border-[#ca8a04]/30 bg-[#fefce8] p-5 dark:border-[#ca8a04]/30 dark:bg-[#422006]/30">
        <div className="flex items-center gap-2 text-[#854d0e] dark:text-[#fef08a]">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
          <div className="rounded-xl bg-white p-3 border border-[#ca8a04]/20 dark:bg-[#141517]">
            <span className="font-bold text-muted-foreground block text-[10px] uppercase">Relief Centre Allotted</span>
            <span className="font-extrabold text-sm text-[#854d0e] dark:text-[#fef08a] uppercase">{c.reliefCentreAllotted}</span>
          </div>
          <div className="rounded-xl bg-white p-3 border border-[#ca8a04]/20 dark:bg-[#141517]">
            <span className="font-bold text-muted-foreground block text-[10px] uppercase">Category Details</span>
            <span className="font-extrabold text-sm text-[#854d0e] dark:text-[#fef08a] uppercase">{c.helpCategory}</span>
          </div>
        </div>
        {c.notes && (
          <p className="mt-2.5 rounded-xl bg-white p-3 text-xs text-muted-foreground border border-[#ca8a04]/20 dark:bg-[#141517] leading-relaxed">
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-2xl border border-[#881337]/20 bg-[#fff1f2]/40 p-5 dark:border-[#eab308]/20 dark:bg-[#422006]/20">
      <div className="flex items-center gap-2 text-[#881337] dark:text-[#facc15]">
        <CheckCircle2 className="h-5 w-5 text-[#881337] dark:text-[#facc15]" />
      ...
      <form onSubmit={handleSubmit} className="mt-4 space-y-4">
        <div>
          <label className="block text-xs font-black text-[#881337] dark:text-[#facc15]">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            <button
              ...
              className={`rounded-xl py-2.5 text-xs font-extrabold transition border ${allotted === "yes" ? "bg-[#881337] text-white border-[#881337] dark:bg-[#eab308] dark:text-[#0a0b0d] dark:border-[#eab308]" : "bg-white text-zinc-700 border-zinc-200 hover:bg-zinc-50 dark:bg-[#1a1c20] dark:text-zinc-200"}`}
            >
              ✓ Yes, Allotted
            </button>
            <button
              ...
              className={`rounded-xl py-2.5 text-xs font-extrabold transition border ${allotted === "no" ? "bg-[#881337] text-white border-[#881337] dark:bg-[#eab308] dark:text-[#0a0b0d] dark:border-[#eab308]" : "bg-white text-zinc-700 border-zinc-200 hover:bg-zinc-50 dark:bg-[#1a1c20] dark:text-zinc-200"}`}
            >
              ✕ No, Not Yet
            </button>
          </div>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            {([
              ...
              <button
                ...
                className={`rounded-xl p-2.5 text-left text-xs font-bold transition border ${helpCategory === val ? "bg-[#881337] text-white border-[#881337] dark:bg-[#eab308] dark:text-[#0a0b0d]" : "bg-white text-zinc-700 border-zinc-200 hover:bg-zinc-50 dark:bg-[#1a1c20] dark:text-zinc-200"}`}
              >
                {label}
              </button>
            ))}
        ...
        <div>
          ...
          <Textarea
            ...
            maxLength={2000}
            className="mt-1.5 min-h-20 bg-white text-xs dark:bg-[#1a1c20]"
          />
        </div>
        ...
        <Button
          ...
          className="w-full rounded-xl bg-[#881337] text-white font-extrabold text-xs h-11 shadow-sm hover:bg-[#9f1239] dark:bg-[#eab308] dark:text-[#0a0b0d] dark:hover:bg-[#facc15]"
        >
...
function LiveRescuerMap({
  ...
  const createEndpointPin = useCallback((kind: "user" | "rescuer" | "hospital") => {
    ...
    if (kind === "hospital") return new PinElement({ background: "#881337", borderColor: "#ffffff", glyph: "H", glyphColor: "#ffffff", scale: 1.15 });
    return kind === "user"
  ...
  const handleLeafletReady = useCallback(async (lMap: any) => {
    ...
    const rescuerIcon = L.divIcon({
      ...
      html: `<div style="background:#881337;color:#fff;border-radius:50%;width:32px;height:32px;display:grid;place-items:center;border:2.5px solid #fff;box-shadow:0 3px 10px rgba(0,0,0,0.35);font-size:16px;transition:transform 0.4s ease;">🛡️</div>`,
      iconSize: [32, 32],
      iconAnchor: [16, 16],
    });

    const rMarker = L.marker([latitude, longitude], { icon: rescuerIcon, title: "Assigned rescuer" });
    rMarker.bindPopup(`<strong style="color:#881337;">${t("Assigned Rescuer")}</strong><br/><span>${t("En route")}</span>`);
    ...
    if (targetPosition) {
      const destIcon = L.divIcon({
        className: "dest-pin",
        html: `<div style="background:${hospital ? "#881337" : "#c94b45"};color:#fff;border-radius:50%;width:32px;height:32px;display:grid;place-items:center;border:2.5px solid #fff;box-shadow:0 3px 10px rgba(0,0,0,0.35);font-size:16px;">${hospital ? "🏥" : "🚨"}</div>`,
        iconSize: [32, 32],
        iconAnchor: [16, 16],
      });

      const sMarker = L.marker([targetPosition.lat, targetPosition.lng], { icon: destIcon, title: "Your SOS location" });
      sMarker.bindPopup(`<strong style="color:#881337;">${hospital?.name || t("Your SOS Location")}</strong>`);
