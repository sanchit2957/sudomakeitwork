...
The requested refinement then removed the device-only Safety Plan entirely. The new 375 px Safety layout places **Verified medical care** and its nearby-hospital card area directly below the emergency actions, followed by the **Local alert** card and then compact flood guidance. The checklist no longer consumes vertical space or appears in source-level UI regressions. TypeScript and all **36** Vitest files / **86** tests still pass.

Each loaded hospital card now includes a compact navigation icon directly beside the hospital name. It is an accessible new-tab Google Maps directions link built from the verified latitude and longitude. Mobile browser verification showed the icon beside the hospital name with the accessible label **Open Google Maps directions to Test hospital 1** and the expected coordinate destination URL. TypeScript and all **36** Vitest files / **86** tests pass.
